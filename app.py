import os, sqlite3, calendar, secrets, json, shutil, io
...
def csrf_protect():
    ...
    # Mobile JSON API uses session auth + JSON bodies (no form _csrf field).
...
def get_setting(c, key, default="0", household_id=1):
    ...
    if r:
        return r["value"]
    # backward compatibility for legacy global settings
...
def get_month_starting_balance(c, month, household_id=1):
    def _compute(m, depth=0):
        ...
        # Use previous month's computed start recursively, then apply projected month delta
        # from paycheck planner inputs (income - bills), not ledger-only history.
...
def projected_checkbook(c, month, household_id=1, carryover_bill_ids=None, start_delta=0.0):
    # Build projected events from starting balance + paychecks + unpaid bills.
...
def assign_paycheck_bucket(bills, paychecks):
    # map each bill id -> (bucket index, paycheck date)
    buckets = {}
    pdays = []
    ...
    for b in bills:
        ...
        # If bill is due before first paycheck in month, treat as prior-month-pay bucket (0).
        if due_day < pdays[0][1]:
            buckets[b["id"]] = (0, 0, "")
            continue

        # Otherwise assign to latest paycheck on/before due date.
        chosen = pdays[0]
        for p in pdays:
            if p[1] <= due_day:
                chosen = p
...
def build_paycheck_plan(bills_for_month, paychecks, starting_balance, buckets):
    # Accounts-style planning: what each paycheck/income covers, including already-paid items.
    groups = {0: []}
    ...
    for i in sorted(groups.keys()):
        if i == 0:
            ...
            out.append({
                ...
                "paycheck_id": 0,
                "date": "",
                "owner": "Last Month Pay",
                "income_type": "last_month",
                "label_base": "Last Month Pay",
                "display_label": "Last Month Pay",
                "paycheck_amount": paycheck_amt,
                "starting_available": available,
                "items": items,
                "ending_balance": running,
                "goes_negative": running < 0,
            })
            carry = running
            continue
        ...
        out.append({
            ...
            "display_label": f"{label_base} {type_index}",
            "paycheck_amount": paycheck_amt,
            "starting_available": available,
            "items": items,
            "ending_balance": running,
...
def auto_apply_recurring_for_month(c, month, household_id=1):
    ...
    # Carry forward recurring bills from prior month into current month (idempotent).
    rows = c.execute(
    ...
    for r in rows:
        # stop recurring once end date month has passed
    ...
    # Generate recurring income from rules for the current month window.
...
def api_mobile_dashboard():
    ...
    paychecks_sum = c.execute(
        "select coalesce(sum(amount),0) s from paychecks where household_id=? and substr(pay_date,1,7)=?",
        (household_id, month),
    ).fetchone()['s']
    expenses_sum = c.execute(
        "select coalesce(sum(amount),0) s from bills where household_id=? and deleted_at is null and substr(due_date,1,7)=?",
        (household_id, month),
    ).fetchone()['s']

    upcoming = c.execute(
        "select id,name,amount,due_date,planned_pay_date,paid,category,note from bills where household_id=? and deleted_at is null and substr(due_date,1,7)=? order by due_date,id limit 30",
        (household_id, month),
    ).fetchall()

    recent_ledger = c.execute(
        "select id,tx_date,label,amount,note from ledger where household_id=? and substr(tx_date,1,7)=? order by tx_date desc,id desc limit 50",
        (household_id, month),
    ).fetchall()

    trips = c.execute(
        "select id,name,due_month,target,saved from trips where household_id=? and deleted_at is null order by due_month,id",
        (household_id,),
    ).fetchall()

    promotions = c.execute(
        "select id,card_name,promo_name,start_date,end_date,balance,completed,note from promotions where household_id=? and deleted_at is null order by end_date,id",
        (household_id,),
    ).fetchall()

    subscriptions = c.execute(
        "select id,name,day_due,payment,recurring,recurring_timeframe,recurring_every_months,yearly_month from subscriptions where household_id=? and deleted_at is null order by day_due,name,id",
        (household_id,),
    ).fetchall()
        ...
        'upcomingBills': [
            {
                ...
                'dueDate': (r['planned_pay_date'] or r['due_date']),
        ...
        'subscriptions': [
            {
                'id': int(r['id']),
                'name': r['name'],
                'dayDue': int(r['day_due'] or 1),
                'payment': float(r['payment'] or 0),
                'recurring': int(r['recurring'] or 0),
                'recurringTimeframe': r['recurring_timeframe'] or 'monthly',
                'recurringEveryMonths': r['recurring_every_months'],
                'yearlyMonth': r['yearly_month'],
            }
...
def api_mobile_theme_set():
    ...
    if theme not in THEMES:
        return api_error('Invalid theme', 400)
    c = conn()
    household_id = active_household_id()
    c.execute(
        "insert into settings(key,value) values(?,?) on conflict(key) do update set value=excluded.value",
        (setting_key(household_id, 'accent_theme'), theme),
    )
    c.commit(); c.close()
...
def api_mobile_bill_paid(item_id):
    ...
    household_id = active_household_id()
    row = c.execute(
        "select * from bills where id=? and household_id=? and deleted_at is null",
    ...
    if int(row['paid'] or 0) == 0:
        ...
        c.execute(
            "update bills set paid=1, paid_date=?, planned_pay_date=? where id=? and household_id=?",
            (paid_on, paid_on, item_id, household_id),
        )
        c.execute(
            "insert into ledger(tx_date,label,amount,note,household_id) values(?,?,?,?,?)",
            (paid_on, row['name'], -abs(float(row['amount'] or 0)), 'marked paid (mobile)', household_id),
        )
...
def api_mobile_bill_postpone(item_id):
    ...
    data = request.get_json(silent=True) or {}
    try:
        postpone_days = int(data.get('days', 7) or 7)
    except Exception:
        postpone_days = 7
    ...
    try:
        base = datetime.strptime(current_target, '%Y-%m-%d')
    except Exception:
        base = local_now()
    ...
    c.execute(
        "update bills set planned_pay_date=?, note=?, paycheck_bucket=NULL where id=? and household_id=?",
        (new_date, new_note, item_id, household_id),
    )
    c.execute(
        "insert into ledger(tx_date,label,amount,note,household_id) values(?,?,?,?,?)",
        (local_now().strftime('%Y-%m-%d'), row['name'], 0, f'postponed (mobile) to {new_date}', household_id),
    )
...
def dashboard():
    ...
    # Keep trip contributions visible in paycheck planner as deducted paid items.
    ...
    # Bills due before first paycheck in month should be carried by prior month pay.
    first_pay_day = None
    if paychecks:
        try:
    ...
    # Next-month carry summary widget: show next month's early bills that come out of this month's last check.
    next_month_carry_bills = []
    next_month_carry_total = 0.0
    next_month_paychecks = c.execute(
        "select pay_date from paychecks where household_id=? and substr(pay_date,1,7)=? order by pay_date, id",
        (household_id, next_month),
    ).fetchall()
    ...
    if next_first_pay_day is not None:
        next_month_bills = c.execute(
            "select * from bills where household_id=? and deleted_at is null and paid=0 and substr(due_date,1,7)=? and lower(coalesce(note,'')) not like '%postponed%' order by due_date, id",
            (household_id, next_month),
        ).fetchall()
    ...
    # Savings contributions (tied to paycheck planner bills using category=savings).
    savings_paid_row = c.execute(
        "select coalesce(sum(amount),0) s from bills where household_id=? and deleted_at is null and lower(coalesce(category,''))='savings' and paid=1 and substr(coalesce(paid_date,due_date),1,7)=?",
        (household_id, month),
    ).fetchone()
    savings_planned_row = c.execute(
        "select coalesce(sum(amount),0) s from bills where household_id=? and deleted_at is null and lower(coalesce(category,''))='savings' and substr(due_date,1,7)=?",
        (household_id, month),
    ).fetchone()
    ...
    # Monthly amount needed to hit trip/activity targets by due date.
    ...
    for b in bills:
        add_event(
            b["planned_pay_date"] or b["due_date"],
            "bill",
            b["name"],
            float(b["amount"]),
            {
                "id": int(b["id"]),
                "due_date": b["due_date"],
                "planned_pay_date": b["planned_pay_date"] or b["due_date"],
                "autopay": 1 if b["autopay"] else 0,
                "paycheck_bucket": b["paycheck_bucket"] if b["paycheck_bucket"] is not None else "",
                "paid": 1 if b["paid"] else 0,
    ...
    for s in subscriptions:
        ...
        if timeframe == "yearly":
            ...
            try:
                interval = int(s["recurring_every_months"] or 1)
            except Exception:
                interval = 1
            if interval < 1:
                interval = 1
            # anchor to current month when first added; fallback to current month
            anchor_ym = month
            try:
                anchor_ym = c.execute("select substr(created_at,1,7) as ym from subscriptions where id=?", (s["id"],)).fetchone()["ym"] or month
            except Exception:
                anchor_ym = month
        ...
        add_event(
            ...
            "subscription",
            s["name"],
            float(s["payment"] or 0),
            {
                "id": int(s["id"]),
                "name": s["name"],
                "day_due": int(s["day_due"] or 1),
                "payment": float(s["payment"] or 0),
                "recurring": int(s["recurring"] or 0),
                "recurring_timeframe": timeframe,
                "recurring_every_months": s["recurring_every_months"] or "",
                "yearly_month": s["yearly_month"] or "",
            },
        )
    ...
    return render_template(
        ...
        unpaid_bills_all=unpaid_bills_all,
        paychecks=paychecks,
        trips=trips,
        promotions=promotions,
        subscriptions=subscriptions,
        recurring_subscriptions=recurring_subscriptions,
        promotions_ending_soon=promotions_ending_soon,
        completed_promotions=completed_promotions,
        due_now_bills=due_now_bills,
        past_due_bills=past_due_bills,
        credit_accounts=credit_accounts,
        loans=loans,
        paid_credit_accounts=paid_credit_accounts,
        ...
        bills_due=bills_due,
        planned_bills_total=planned_bills_total,
        income=income,
        balance=balance,
        buckets=buckets,
        projection=projection,
        paycheck_plan=paycheck_plan,
        month_start=month_start,
        remaining_unpaid_total=remaining_unpaid_total,
        current_risk_balance=current_risk_balance,
        next_month_carry_bills=next_month_carry_bills,
        next_month_carry_total=next_month_carry_total,
        carry_after_balance=carry_after_balance,
        month=month,
        ...
        view=view,
        page=page,
        paycheck_rules=paycheck_rules,
        recurring_bills=recurring_bills,
        categories=categories,
        theme=theme,
        theme_name=theme_name,
        household_name=household_name,
        dashboard_note_text=dashboard_note_text,
        savings_paid_month=savings_paid_month,
        savings_planned_month=savings_planned_month,
...
def entries_add():
    ...
    if entry_type == "income":
        ...
        if income_kind not in ("paycheck", "other"):
            income_kind = "paycheck"
        owner_label = (request.form.get("owner", "") or "").strip()
        if income_kind == "other" and not owner_label:
            owner_label = "Other Income"
        c.execute(
            "insert into paychecks(owner,pay_date,amount,income_type,household_id) values(?,?,?,?,?)",
            (
                owner_label,
                request.form.get("pay_date", ""),
                float(request.form.get("income_amount", 0) or 0),
                income_kind,
                household_id,
            ),
        )
    ...
    if entry_type == "trip_contribution":
        try:
            trip_id = int(request.form.get("contrib_trip_id", "0") or 0)
            amount = abs(float(request.form.get("contrib_amount", 0) or 0))
        except Exception:
            trip_id, amount = 0, 0
        if trip_id > 0 and amount > 0:
            ...
            if tr is not None:
                ...
                c.execute(
                    "insert into ledger(tx_date,label,amount,note,household_id) values(?,?,?,?,?)",
                    (
                        request.form.get("contrib_date", datetime.now().strftime("%Y-%m-%d")),
                        f"Trip Contribution - {tr['name']}",
                        -amount,
                        "trip contribution",
                        household_id,
                    ),
    ...
    try:
        amount = float(str(amount_raw or 0).replace(',', '').strip() or 0)
    except Exception:
        amount = 0.0
    category = request.form.get("category", "bill")
    trip_id_raw = request.form.get("trip_id", "")

    # If adding a trip-category expense tied to a saved trip, allow quick-select and decrement saved.
    ...
    if recurring_timeframe not in ("monthly", "yearly", "specified_months"):
        recurring_timeframe = "monthly"
    try:
        recurring_every_months = int((request.form.get("recurring_every_months", "") or "").strip())
        if recurring_every_months < 1:
            recurring_every_months = None
    except Exception:
        recurring_every_months = None
    ...
    try:
        if recurring_end_date:
            _ = datetime.strptime(recurring_end_date, "%Y-%m-%d")
        else:
    ...
    c.execute(
        "insert into bills(name,due_date,planned_pay_date,amount,paid,category,recurring,recurring_timeframe,recurring_every_months,recurring_end_date,autopay,household_id) values(?,?,?,?,?,?,?,?,?,?,?,?)",
        (
            name,
            due_date_val,
            planned_pay_date_val,
            amount,
            0,
            category,
            recurring_flag,
            recurring_timeframe,
            recurring_every_months if recurring_timeframe == "specified_months" else None,
            recurring_end_date,
            1 if request.form.get("autopay") == "on" else 0,
            household_id,
        ),
    )
    c.commit(); c.close()
...
def bills_update(item_id):
    ...
    if row is not None:
        ...
        try:
            _ = datetime.strptime(new_due, "%Y-%m-%d")
        except Exception:
            new_due = row["due_date"]
        try:
            new_amt = float(request.form.get("amount", row["amount"]))
        except Exception:
        ...
        try:
            ...
            recurring_end_date = (row["recurring_end_date"] if "recurring_end_date" in row.keys() else None)
        ...
        if new_paid:
            new_paid_date = paid_date_raw or (row["paid_date"] if "paid_date" in row.keys() else "") or local_now().strftime("%Y-%m-%d")
        else:
            new_paid_date = None
        pb_raw = request.form.get("paycheck_bucket", "")
        try:
            new_pb = int(pb_raw) if str(pb_raw).strip() else None
        except Exception:
            new_pb = None

        # If planned pay date changed, re-enable auto paycheck assignment.
        prev_planned = (row["planned_pay_date"] if "planned_pay_date" in row.keys() else "") or row["due_date"]
        if new_planned != prev_planned:
            new_pb = None
...
def bills_postpone(item_id):
    ...
    if row is not None:
        ...
        if remove_postpone:
            cleaned_lines = []
            for ln in existing_note.splitlines():
                if 'postponed' in ln.lower():
                    continue
            ...
            if postpone_date:
                try:
                    _ = datetime.strptime(postpone_date, "%Y-%m-%d")
                    new_planned = postpone_date
                except Exception:
                    pass

            stamp = local_now().strftime('%m-%d-%Y')
            tagged = f"[{stamp}] Postponed"
            if postpone_note:
                tagged += f": {postpone_note}"
            new_note = (existing_note + "\n" + tagged).strip() if existing_note else tagged
...
def bills_delete(item_id):
    ...
    if request.method == 'GET':
        month = normalize_month(request.args.get('month', datetime.now().strftime('%Y-%m')))
        view = request.args.get('view', 'paycheck')
        page = request.args.get('page', 'entries')
...
def bills_toggle(item_id):
    ...
    if row is not None and row["paid"] == 0:
        ...
        c.execute(
            "insert into ledger(tx_date,label,amount,note,household_id) values(?,?,?,?,?)",
            (paid_on, row["name"], -abs(float(row["amount"])), "marked paid", household_id),
        )
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def bills_toggle_get_fallback(item_id):
    # Safety fallback: if browser reloads a prior POST URL, redirect cleanly.
    if not authed():
        return redirect(url_for("login"))
    month = normalize_month(request.args.get("month", datetime.now().strftime("%Y-%m")))
    view = request.args.get("view", "paycheck")
...
def paychecks_add():
    ...
    c = conn()
    household_id = active_household_id()
    c.execute("insert into paychecks(owner,pay_date,amount,income_type,household_id) values(?,?,?,?,?)",
              (request.form["owner"], request.form["pay_date"], float(request.form["amount"]), "paycheck", household_id))
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def paychecks_update(item_id):
    ...
    owner = request.form.get('owner', row['owner'])
    pay_date = request.form.get('pay_date', row['pay_date'])
    ...
    if income_type not in ('paycheck', 'other'):
        income_type = 'paycheck'
    if income_type == 'other':
        owner = 'Income'

    c.execute('update paychecks set owner=?, pay_date=?, amount=?, income_type=? where id=? and household_id=?',
              (owner, pay_date, amount, income_type, item_id, household_id))
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def paycheck_rules_add():
    if not authed(): return redirect(url_for('login'))
    month = normalize_month(request.form.get('month', datetime.now().strftime('%Y-%m')))
    view = request.form.get('view', 'checklist')
    c = conn()
    household_id = active_household_id()
    ...
    if cadence not in ('weekly', 'biweekly', 'monthly', 'specified_day'):
        cadence = 'biweekly'
    day_raw = (request.form.get('day_of_month', '') or '').strip()
    try:
        day_of_month = int(day_raw) if day_raw else None
    except Exception:
        day_of_month = None
    if cadence == 'specified_day' and (day_of_month is None or day_of_month < 1 or day_of_month > 31):
        day_of_month = 1
    ...
    if cadence == 'specified_day':
        next_date = _date_for_month_day(month, day_of_month)
    c.execute('insert into paycheck_rules(owner,amount,cadence,next_date,active,household_id,day_of_month,timeframe,months_mask) values(?,?,?,?,1,?,?,?,?)', (
...
@app.post('/paycheck-rules/update/<int:item_id>')
def paycheck_rules_update(item_id):
    if not authed(): return redirect(url_for('login'))
    month = normalize_month(request.form.get('month', datetime.now().strftime('%Y-%m')))
    c = conn()
    household_id = active_household_id()
    row = c.execute('select * from paycheck_rules where id=? and household_id=? and active=1', (item_id, household_id)).fetchone()
    if row is None:
        c.close()
        return redirect_dashboard_from_form(request.form)
    ...
    if cadence == 'specified_day':
        next_date = _date_for_month_day(month, day_of_month)
    if owner and next_date:
        c.execute(
            'update paycheck_rules set owner=?, amount=?, cadence=?, next_date=?, day_of_month=?, timeframe=?, months_mask=? where id=? and household_id=? and active=1',
            (owner, amount, cadence, next_date, day_of_month, timeframe, months_mask, item_id, household_id),
        )
        # remove previously generated paycheck instances from all months for this rule signature
        c.execute(
            'delete from paychecks where household_id=? and income_type=? and owner=? and amount=?',
            (household_id, 'paycheck', old_owner, old_amount),
        )
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def paycheck_rules_delete(item_id):
    ...
    if row is not None:
        c.execute(
            'delete from paychecks where household_id=? and income_type=? and owner=? and amount=?',
            (household_id, 'paycheck', row['owner'], float(row['amount'] or 0)),
        )
...
def paycheck_rules_bulk_update():
    ...
    cadences = request.form.getlist('cadence')
    next_dates = request.form.getlist('next_date')
    days_of_month = request.form.getlist('day_of_month')
    timeframes = request.form.getlist('timeframe')
    months_masks = request.form.getlist('months_mask')
    ...
    for rid, owner, amount_raw, cadence, next_date, day_raw, timeframe, months_mask in rows:
        ...
        try:
            ...
            amount = 0
        try:
            day_of_month = int((day_raw or '').strip()) if (day_raw or '').strip() else None
        except Exception:
            day_of_month = None
        if cadence == 'specified_day' and (day_of_month is None or day_of_month < 1 or day_of_month > 31):
            day_of_month = 1
        ...
        c.execute(
            'delete from paychecks where household_id=? and income_type=? and owner=? and amount=?',
            (household_id, 'paycheck', old['owner'], float(old['amount'] or 0)),
        )
...
def ledger_add():
    ...
    c = conn()
    household_id = active_household_id()
    c.execute("insert into ledger(tx_date,label,amount,note,household_id) values(?,?,?,?,?)",
              (request.form["tx_date"], request.form["label"], float(request.form["amount"]), request.form.get("note",""), household_id))
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def ledger_update(item_id):
    ...
    if row is not None:
      ...
      try:
        amount = float(request.form.get('amount', row['amount']))
      except Exception:
...
def ledger_delete(item_id):
    ...
    row = c.execute('select * from ledger where id=? and household_id=?', (item_id, household_id)).fetchone()
    if row is not None:
        # If this is a trip contribution, rollback saved amount on that trip.
        if (row['note'] or '') == 'trip contribution' and float(row['amount'] or 0) < 0:
            label = row['label'] or ''
            prefix = 'Trip Contribution - '
...
def trips_add():
    ...
    if len(due_val) == 7:
        due_val = f"{due_val}-01"
    c.execute("insert into trips(name,due_month,target,saved,household_id) values(?,?,?,?,?)",
              (request.form["name"], due_val, float(request.form["target"]), float(request.form.get("saved",0)), household_id))
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def trips_update(item_id):
    ...
    if row is not None:
      ...
      if len(str(due_month)) == 7:
        due_month = f"{due_month}-01"
...
def trips_contribute(item_id):
    ...
    if row is not None:
      ...
      if amt > 0:
        ...
        c.execute('insert into ledger(tx_date,label,amount,note,household_id) values(?,?,?,?,?)', (
          request.form.get('paid_date', datetime.now().strftime('%Y-%m-%d')),
          f"Trip Contribution - {row['name']}",
          -amt,
          'trip contribution',
          household_id,
        ))
...
def promotions_add():
    ...
    if card_name and promo_name and end_date:
        c.execute(
            'insert into promotions(card_name,promo_name,start_date,end_date,balance,note,household_id) values(?,?,?,?,?,?,?)',
            (card_name, promo_name, start_date, end_date, balance, note, household_id),
        )
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def promotions_update(item_id):
    ...
    if row is not None:
        ...
        try:
            balance = float(request.form.get('balance', row['balance']) or row['balance'])
        except Exception:
            balance = float(row['balance'])
        c.execute(
            'update promotions set card_name=?, promo_name=?, start_date=?, end_date=?, balance=?, note=? where id=? and household_id=?',
            (card_name, promo_name, start_date, end_date, balance, note, item_id, household_id),
        )
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def subscriptions_add():
    ...
    try:
        day_due = int(request.form.get('day_due', 1) or 1)
    except Exception:
        day_due = 1
    day_due = max(1, min(31, day_due))
    try:
        payment = float(request.form.get('payment', 0) or 0)
    except Exception:
        payment = 0
    ...
    try:
        ...
        if yearly_month < 1 or yearly_month > 12:
            yearly_month = None
    except Exception:
        yearly_month = None
    if name:
...
def credit_add():
    ...
    if card_name:
        c.execute(
            'insert into credit_accounts(card_name,interest_rate,credit_limit,household_id) values(?,?,?,?)',
            (card_name, interest_rate, credit_limit, household_id),
        )
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def credit_update(item_id):
    ...
    if card_name:
        c.execute(
            'update credit_accounts set card_name=?, interest_rate=?, credit_limit=?, paid_off=? where id=? and household_id=? and deleted_at is null',
            (card_name, interest_rate, credit_limit, paid_off, item_id, household_id),
        )
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def loans_add():
    ...
    if loan_name:
        c.execute(
            'insert into loans(loan_name,interest_rate,loan_amount,note,end_date,household_id) values(?,?,?,?,?,?)',
            (loan_name, interest_rate, loan_amount, loan_note, end_date, household_id),
        )
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def loans_update(item_id):
    ...
    if loan_name:
        c.execute(
            'update loans set loan_name=?, interest_rate=?, loan_amount=?, note=?, end_date=? where id=? and household_id=? and deleted_at is null',
            (loan_name, interest_rate, loan_amount, loan_note, end_date, item_id, household_id),
        )
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def household_invite():
    ...
    c.execute(
        'insert into household_invites(household_id,email,role,token,expires_at,created_by_user_id,created_at) values(?,?,?,?,?,?,?)',
        (household_id, email, role, token, exp, current_user_id() or None, datetime.now().strftime('%Y-%m-%d %H:%M:%S')),
    )
    c.commit(); c.close()
...
def household_accept(token):
    ...
    if not inv:
        c.close()
        return 'Invite is invalid or already used.', 400
    if datetime.strptime(inv['expires_at'], '%Y-%m-%d %H:%M:%S') < datetime.now():
        c.close()
    ...
    if (user['email'] or '').strip().lower() != (inv['email'] or '').strip().lower():
        c.close()
        return 'Invite email does not match your account.', 403
...
def export_excel():
    ...
    try:
        ...
        c.close()
        return ("Excel export dependency missing. Install openpyxl.", 500)
    ...
    ws.append(["NorthStar Ledger Export"])
    ws.append(["Household", household_name])
    ...
    for sheet_name, query in exports:
        ...
        sh.append(headers)
        for r in rows:
            sh.append([r[k] for k in headers])
...
def dashboard_note_update():
    ...
    c.execute(
        "insert into dashboard_notes(household_id,note_text,updated_at) values(?,?,?) "
        "on conflict(household_id) do update set note_text=excluded.note_text, updated_at=excluded.updated_at",
        (household_id, note_text, now_ts),
    )
    c.commit(); c.close()
    return redirect_dashboard_from_form(request.form)
...
def settings_starting_balance():
    ...
    page = request.form.get("page", "entries")
...
def rollover():
    ...
    # Default behavior: apply recurring bills + income to the currently viewed month.
    tgt = normalize_month(request.form.get("month", datetime.now().strftime("%Y-%m")))
    src = request.form.get("source_month", "")  # optional legacy override
    if len(src) != 7:
        src, _ = month_prev_next(tgt)

    c = conn()
    household_id = active_household_id()
    rows = c.execute(
        """
        select id,name,due_date,amount,category,recurring,autopay,
               coalesce(recurring_timeframe,'monthly') as recurring_timeframe,
               recurring_every_months, recurring_end_date
        from bills
        where household_id=? and deleted_at is null and recurring=1
        """,
        (household_id,),
    ).fetchall()
    ...
    for r in rows:
        ...
        if timeframe == "yearly":
            ...
            try:
                interval = int(r["recurring_every_months"] or 1)
            except Exception:
                interval = 1
            if interval < 1:
                interval = 1
        ...
        # Respect optional recurring end date: stop generating in months after end date month.
        end_ym = str(r["recurring_end_date"] or "")[:7]
        if len(end_ym) == 7 and end_ym < tgt:
            continue
        ...
        if not exists:
            c.execute(
                "insert into bills(name,due_date,amount,paid,category,recurring,recurring_timeframe,recurring_every_months,recurring_end_date,autopay,household_id) values(?,?,?,?,?,?,?,?,?,?,?)",
                (r["name"], new_due, r["amount"], 0, r["category"], r["recurring"], timeframe, (r["recurring_every_months"] if timeframe=='specified_months' else None), r["recurring_end_date"], r["autopay"], household_id),
            )

    # include recurring paycheck generation in same action
...
def import_seed():
    # quick import from assistant_budget_system CSV folder
    if not authed(): return redirect(url_for("login"))
    base = os.path.join(os.path.dirname(__file__), "..", "assistant_budget_system")
    ...
    # settings
    for r in read_csv(os.path.join(base, "settings.csv")):
        k = (r.get("key") or "").strip()
        v = (r.get("value") or "").strip()
        if k in ("starting_balance", "min_buffer"):
            c.execute("insert into settings(key,value) values(?,?) on conflict(key) do update set value=excluded.value", (k, v))

    # paychecks
    for r in read_csv(os.path.join(base, "income.csv")):
        if (r.get("account","").lower() != "bills"):
            continue
        c.execute("insert into paychecks(owner,pay_date,amount,income_type,household_id) values(?,?,?,?,?)", (r.get("owner",""), r.get("date",""), float(r.get("amount") or 0), "paycheck", household_id))

    # bills
    for r in read_csv(os.path.join(base, "bills.csv")):
        if (r.get("active","").lower() != "yes"):
            continue
        day = int(float(r.get("due_day") or 1))
        month = datetime.now().strftime("%Y-%m")
        due = f"{month}-{day:02d}"
        c.execute(
            "insert into bills(name,due_date,amount,paid,category,recurring,autopay,household_id) values(?,?,?,?,?,?,?,?)",
            (r.get("name",""), due, float(r.get("amount") or 0), 0, r.get("category","bill"), 1, 1 if str(r.get("autopay","")).lower() in ("yes","true","1") else 0, household_id),
        )
