...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Auto-generate recurring bills for the current month on every page load
  const _initMonth = `${plannerYear}-${String(plannerMonth + 1).padStart(2, '0')}`;
  autoGenerateForMonth(_initMonth).then(() => _reRenderActiveTab());
...
function globalPrevMonth() {
  ...
  syncAllMonthLabels();
  _reRenderActiveTab();
  const ms = `${plannerYear}-${String(plannerMonth + 1).padStart(2, '0')}`;
  autoGenerateForMonth(ms).then(() => _reRenderActiveTab());
}
...
function renderPlanner() {
  ...
  const unassigned = state.bills.filter(b => !b.paycheck_id && b.month === monthStr);
...
function openImportRecurring() {
  ...
  // Prefer recurring_templates; fall back to legacy is_template bills
  let templates = state.recurringTemplates.length ? state.recurringTemplates : null;
  if (!templates) {
    const seen = {};
    state.bills.filter(b => b.is_recurring && b.is_template).sort((a, b) => a.id - b.id)
  ...
  function dueInMonth(template) {
    // due_day is the new field (integer); fall back to parsing due_date for legacy
    const dueDay = template.due_day != null
      ? template.due_day
      : (template.due_date ? parseInt(template.due_date.slice(8, 10), 10) : null);
    if (!dueDay) return `${ty}-${String(tm).padStart(2,'0')}-01`;
...
async function confirmImportRecurring(toAdd) {
  ...
  try {
    for (const { template, dueDate } of toAdd) {
      // Prefer assigning to a paycheck within the target month so the imported bill
      // appears in the correct month's planner. Fall back to the global logic only
      // when the target month has no paychecks at all.
      const paycheckId = autoAssignPaycheckInMonth(dueDate, monthStr);
...
async function saveEditBill() {
  ...
  const bill        = state.bills.find(b => b.id === id);
  const oldRecurring = bill ? !!bill.is_recurring : false;
  ...
  listEl.innerHTML = `
    <div class="debt-section">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;padding:0 0.25rem;">
        <span style="font-size:0.85rem;color:var(--text-md);">${templates.length} recurring bill${templates.length !== 1 ? 's' : ''}</span>
        <span style="font-size:0.85rem;color:var(--text-md);">Est. monthly total: <strong style="color:var(--danger);">$${fmt(totalMonthly)}</strong></span>
      </div>
      <table class="debt-table">
        <thead>
          <tr>
        ...
        <tbody>
          ${templates.map(t => {
            ...
            const cat  = (t.category  || 'bill').charAt(0).toUpperCase()    + (t.category  || 'bill').slice(1);
            return `
          <tr>
...
function openEditTemplate(id) {
  ...
  if (!t) return;
  document.getElementById('modal-template-title').textContent = 'Edit Recurring Bill';
...
function openNewTemplate() {
  document.getElementById('modal-template-title').textContent = 'New Recurring Bill';
...
async function saveTemplate() {
  ...
  const dueDayVal = document.getElementById('template-due-day').value;
  const payload = {
  ...
  const result = id
    ? await api('PUT',  `/api/recurring-templates/${id}`, payload)
    : await api('POST', '/api/recurring-templates', payload);

  if (result && result.error) { alert(result.error); return; }

  if (id) {
    const idx = state.recurringTemplates.findIndex(t => t.id === parseInt(id, 10));
    if (idx > -1) state.recurringTemplates[idx] = result;
    else          state.recurringTemplates.push(result);
  } else {
    state.recurringTemplates.push(result);
  }
...
async function deleteTemplate() {
  const id = document.getElementById('template-id').value;
  if (!id) return;
  if (!confirm('Stop this recurring bill? Existing instances are kept but no new ones will generate.')) return;

  const result = await api('DELETE', `/api/recurring-templates/${id}`, null);
  if (result && result.error) { alert(result.error); return; }
...
// Like autoAssignPaycheck but prefers paychecks from targetMonthStr (YYYY-MM).
// This keeps imported/generated bills visible in the correct month's planner.
// Falls back to the global logic only when the target month has no paychecks.
function autoAssignPaycheckInMonth(dueDate, targetMonthStr) {
  if (!dueDate || !state.paychecks.length) return null;
  const monthChecks = [...state.paychecks]
    .filter(p => p.date && p.date.slice(0, 7) === targetMonthStr)
    .sort((a, b) => a.date.localeCompare(b.date));

  if (monthChecks.length) {
    // Most recent paycheck in the target month on or before the due date
    let best = null;
    for (const p of monthChecks) {
      if (p.date <= dueDate) best = p;
    }
    // If due date is before the first paycheck of the month, use the first one
    if (!best) best = monthChecks[0];
    return best.id;
  }

  // No paychecks in the target month — fall back to original global logic
  return autoAssignPaycheck(dueDate);
}
