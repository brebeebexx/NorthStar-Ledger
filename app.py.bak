...
def add_paycheck():
    ...
    db   = get_db()
    db.execute('INSERT INTO paychecks (user_id, date, amount, notes) VALUES (?,?,?,?)',
               (uid, data['date'], data['amount'], data.get('notes', '')))
    db.commit()
...
def manage_paycheck(pid):
    ...
    data = request.get_json()
    db.execute('UPDATE paychecks SET date=?, amount=?, notes=? WHERE id=?',
               (data['date'], data['amount'], data.get('notes', ''), pid))
    db.commit()
